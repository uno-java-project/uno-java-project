...
public class GamePacket implements Serializable {
    ...
    public GamePacket(String userID, int mode, String message, ImageIcon image, UnoGame uno, int roomCount, int roomReady, int roomNum) {
        this.userID = userID;
        this.mode = mode;
    ...
    public GamePacket(String userID, int mode, String message, ImageIcon image, int roomNum) {
        this(userID, mode, message, image, null, 0, 0, roomNum);
    }

    // 방 번호만 전달하는 생성자
    public GamePacket(String userID, int mode, int roomNum) {
        this(userID, mode, null, null, null, 0, 0, roomNum);
    }

    // 방 수와 방 번호만 전달하는 생성자
    public GamePacket(String userID, int mode, int roomCount, int roomNum) {
        this(userID, mode, null, null, null, roomCount, 0, roomNum);
    }

    public GamePacket(String userID, int mode, Integer roomReady, int roomNum) {
        this(userID, mode, null, null, null, 0, roomReady, roomNum);
    }

    // 메시지와 방 번호만 전달하는 생성자
    public GamePacket(String userID, int mode, String message, int roomNum) {
        this(userID, mode, message, null, null, 0, 0, roomNum);
    }

    // UnoGame 객체와 방 번호만 전달하는 생성자
    public GamePacket(String userID, int mode, UnoGame uno, int roomNum) {
        this(userID, mode, null, null, uno, 0, 0, roomNum);
    }
